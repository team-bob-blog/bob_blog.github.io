...
# 설정: 파일 경로 및 데이터 
# ==============================================================================
# Docs 파일 경로 (README.md 파일이 /docs/ 경로를 담당한다고 가정)
DOCS_FILE_PATH = 'docs/README.md' 

# 한 페이지에 표시할 포스트 수 (Liquid에서 직접 사용)
POSTS_PER_PAGE = 10 
...
def generate_docs_content():
    """README.md 파일에 삽입될 Liquid 기반의 저자별 목록 콘텐츠를 생성합니다."""
...
"""
    ...
    # Liquid Body Content (페이지 목록 구현 및 매칭 오류 수정)
    liquid_body = """
<style>
/* ... (스타일 코드는 이전과 동일하게 유지) ... */
.author-section {
...
def main():
    """README.md 파일을 생성 또는 덮어씁니다."""
    
    docs_content = generate_docs_content()
    
    try:
        if not os.path.exists(os.path.dirname(DOCS_FILE_PATH)):
            os.makedirs(os.path.dirname(DOCS_FILE_PATH), exist_ok=True)
            
        with open(DOCS_FILE_PATH, 'w', encoding='utf-8') as f:
            f.write(docs_content)
        print(f"✅ 문서 파일 생성/업데이트 완료: {DOCS_FILE_PATH}")
    except Exception as e:
        print(f"❌ 문서 파일 생성 실패 ({DOCS_FILE_PATH}): {e}")
